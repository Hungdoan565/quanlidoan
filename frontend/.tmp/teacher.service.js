...
var isValidUrl = supabaseUrl && supabaseUrl.startsWith("https://");
if (!isValidUrl || !supabaseAnonKey) {
  console.warn(
    "\u26A0\uFE0F Supabase ch\u01B0a \u0111\u01B0\u1EE3c c\u1EA5u h\xECnh. Vui l\xF2ng c\u1EADp nh\u1EADt file .env v\u1EDBi th\xF4ng tin Supabase th\u1EF1c t\u1EBF.",
    "\n\u2192 VITE_SUPABASE_URL=https://your-project.supabase.co",
    "\n\u2192 VITE_SUPABASE_ANON_KEY=your-anon-key"
  );
}
var supabase = isValidUrl && supabaseAnonKey ? createClient(supabaseUrl, supabaseAnonKey) : {
  ...
  auth: {
    ...
    signInWithPassword: async () => {
      throw new Error("Supabase ch\u01B0a \u0111\u01B0\u1EE3c c\u1EA5u h\xECnh");
    },
    signOut: async () => {
    }
  },
  from: () => ({
    ...
    insert: () => ({ data: null, error: new Error("Supabase ch\u01B0a \u0111\u01B0\u1EE3c c\u1EA5u h\xECnh") }),
    update: () => ({ data: null, error: new Error("Supabase ch\u01B0a \u0111\u01B0\u1EE3c c\u1EA5u h\xECnh") }),
    delete: () => ({ data: null, error: new Error("Supabase ch\u01B0a \u0111\u01B0\u1EE3c c\u1EA5u h\xECnh") })
  })
};
...
var teacherService = {
  ...
  getTeacherDashboardStats: async (teacherId) => {
    try {
      const { data: myClasses } = await supabase.from("classes").select("id").eq("advisor_id", teacherId);
      const classIds = myClasses?.map((c) => c.id) || [];
      let guidingStudents = 0;
      if (classIds.length > 0) {
        const { count } = await supabase.from("class_students").select("*", { count: "exact", head: true }).in("class_id", classIds);
        guidingStudents = count || 0;
      }
      const { count: pendingApproval } = await supabase.from("topics").select("*", { count: "exact", head: true }).eq("advisor_id", teacherId).eq("status", "pending");
      const { count: pendingGrades } = await supabase.from("topics").select("*", { count: "exact", head: true }).eq("advisor_id", teacherId).eq("status", "submitted");
      const { count: completedTopics } = await supabase.from("topics").select("*", { count: "exact", head: true }).eq("advisor_id", teacherId).in("status", ["completed", "defended"]);
      return {
        guidingStudents,
  ...
  getTodoList: async (teacherId) => {
    try {
      const todos = [];
      const { data: pendingTopics } = await supabase.from("topics").select(`
                    id, title, created_at,
                    student:profiles!topics_student_id_fkey(full_name)
                `).eq("advisor_id", teacherId).eq("status", "pending").order("created_at", { ascending: true }).limit(5);
      pendingTopics?.forEach((topic) => {
        todos.push({
          id: `pending-${topic.id}`,
          type: "pending_approval",
          icon: "AlertCircle",
          title: "\u0110\u1EC1 t\xE0i ch\u1EDD duy\u1EC7t",
      ...
      const { data: submittedTopics } = await supabase.from("topics").select(`
                    id, title,
                    student:profiles!topics_student_id_fkey(full_name)
                `).eq("advisor_id", teacherId).eq("status", "submitted").limit(5);
      submittedTopics?.forEach((topic) => {
        todos.push({
          id: `grading-${topic.id}`,
          type: "pending_grading",
          icon: "FileText",
          title: "B\xE1o c\xE1o c\u1EA7n ch\u1EA5m",
          description: `${topic.student?.full_name}: "${topic.title}"`,
  ...
  getGuidingStudents: async (teacherId) => {
    try {
      const { data: myClasses } = await supabase.from("classes").select("id, code, name").eq("advisor_id", teacherId);
      ...
      const { data: classStudents, error: csError } = await supabase.from("class_students").select(`
                    class_id,
                    created_at,
                    student:profiles(
                        id, full_name, email, student_code, avatar_url
                    )
                `).in("class_id", classIds).order("created_at", { ascending: true });
      if (csError) throw csError;
      const { data: topics } = await supabase.from("topics").select("id, title, status, updated_at, student_id, class_id").in("class_id", classIds);
      const topicMap = {};
      topics?.forEach((t) => {
  ...
  getTemplateTopics: async (teacherId) => {
    try {
      const { data, error } = await supabase.from("template_topics").select("*").eq("teacher_id", teacherId).order("created_at", { ascending: false });
  ...
  getMyPendingTopics: async () => {
    ...
    const { data, error } = await supabase.from("topics").select(`
            ...
            `).eq("advisor_id", user.id).eq("status", "pending").order("created_at", { ascending: true });
    if (error) throw error;
    return data || [];
  },
  /**
  ...
  bulkApproveTopics: async (topicIds) => {
    if (!topicIds || topicIds.length === 0) {
      throw new Error("Kh\xF4ng c\xF3 \u0111\u1EC1 t\xE0i n\xE0o \u0111\u01B0\u1EE3c ch\u1ECDn");
    }
    const { data, error } = await supabase.from("topics").update({
      status: "approved",
      approved_at: (/* @__PURE__ */ new Date()).toISOString(),
  ...
  requestRevision: async (topicId, note) => {
    if (!note?.trim()) {
      throw new Error("Vui l\xF2ng nh\u1EADp n\u1ED9i dung y\xEAu c\u1EA7u ch\u1EC9nh s\u1EEDa");
    }
    const { data, error } = await supabase.from("topics").update({
      status: "revision",
  ...
  rejectTopic: async (topicId, reason) => {
    if (!reason?.trim()) {
      throw new Error("Vui l\xF2ng nh\u1EADp l\xFD do t\u1EEB ch\u1ED1i");
    }
    const { data, error } = await supabase.from("topics").update({
      status: "rejected",
  ...
  getMyClasses: async (teacherId) => {
    try {
      const { data: classes, error } = await supabase.from("classes").select(`
                ...
                `).eq("advisor_id", teacherId).order("created_at", { ascending: false });
      if (error) throw error;
      const classIds = classes?.map((c) => c.id) || [];
      let topicStats = {};
      if (classIds.length > 0) {
        const { data: topics } = await supabase.from("topics").select("class_id, status").in("class_id", classIds);
        topics?.forEach((t) => {
  ...
  getClassStudents: async (classId, teacherId) => {
    try {
      const { data: classData, error: classError } = await supabase.from("classes").select(`
                ...
                `).eq("id", classId).eq("advisor_id", teacherId).single();
      if (classError) {
        if (classError.code === "PGRST116") {
          throw new Error("B\u1EA1n kh\xF4ng c\xF3 quy\u1EC1n xem l\u1EDBp n\xE0y");
        }
        throw classError;
      }
      ...
      if (studentIds.length > 0) {
        const { data: topics } = await supabase.from("topics").select("id, student_id, title, status, updated_at").eq("class_id", classId);
        topics?.forEach((t) => {
      ...
      return {
        ...classData,
        students: classData?.class_students?.sort((a, b) => new Date(a.created_at) - new Date(b.created_at))?.map((cs) => ({
          ...cs.student,
  ...
  getAllMyStudents: async (teacherId) => {
    try {
      ...
      if (csError) throw csError;
      const { data: topics, error: topicError } = await supabase.from("topics").select(`
      ...
      const students = classStudents?.map((cs) => {
        ...
        return {
          ...
          student: cs.student
          // Keep student reference for card display
        };
      }).filter((s) => s.student?.id) || [];
      return {
        students,
  ...
  getMyTopicStats: async () => {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) throw new Error("Not authenticated");
    const { data, error } = await supabase.from("topics").select("status").eq("advisor_id", user.id);
    if (error) throw error;
    const stats = {
      total: data?.length || 0,
      pending: 0,
...
var teacher_service_default = teacherService;
export {
  teacher_service_default as default,
  teacherService
};
